...
describe('transitionRefresherRuntimeState', () => {
  it.each([
    [true, 'enabled'],
    [false, 'disabled'],
  ] as const)('initializes enabled=%s with %s intent and idle owned state', (enabled, intent) => {
  ...
  it.each(['paused', 'disabled'] as const)(
    '%s intent preserves owned timer handles until the caller clears them',
    (type) => {
  ...
  it('enables without changing timing or execution ownership', () => {
  ...
  it('replaces and clears interval timing without changing intent or execution', () => {
    let state = transitionRefresherRuntimeState(initialRefresherRuntimeState(true), {
      type: 'refresh-started',
      execution: execution(1),
    });
  ...
  it('finishes only the execution that owns the completion id', () => {
  ...
  it('finishes only the cooldown that owns the timer and retains the interval', () => {
    let state = transitionRefresherRuntimeState(initialRefresherRuntimeState(true), {
      type: 'idle',
      intervalTimer: 17,
    });
