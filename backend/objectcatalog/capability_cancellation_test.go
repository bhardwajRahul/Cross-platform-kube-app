...
func TestCatalogCapabilityFailureDoesNotReportCanceledSiblingRequests(t *testing.T) {
	...
	var requests, arrived atomic.Int32
	...
	transport := capabilityFailureTransport(func(r *http.Request) (*http.Response, error) {
		// Fail the initial batch; fallback cancellation uses Go's real transport.
	...
	require.NoError(t, ctx.Err(), "the parent remains active while the failed task cancels its siblings")
